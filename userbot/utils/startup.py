...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jmthon.tgbot.send_file(
                ...
                caption="⌯︙**بــوت نـاروتو يـعـمـل بـنـجـاح**  ✅ \n⌯︙**قـنـاة الـسـورس**  :  @MusicElkeatib",
                buttons=[(Button.url("جـروب نـاروتو", "https://t.me/Music54Elkeatib"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت نـاروتو تمت بنجاح**"
            )
...
async def autojo():
    try:
        await jmthon(JoinChannelRequest("@MusicElkeatib"))
        if gvar("AUTOEO") is False:
            ...
            try:
                ...
                pass
            try:
                await jmthon(JoinChannelRequest("@SourceRiley"))
            except BaseException:
                pass
    except BaseException:
        pass
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @MusicElkeatib"
